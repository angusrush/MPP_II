...
\begin{definition}
  ...
  \begin{equation}
    ...
    \end{tikzcd},
  \end{equation}
  is \defn{left Kan} if it satisfies the following conditions.
  \begin{itemize}
    \item For all objects $y \in Y$, the category $\category{C}$ admits all colimits of shape $X_{/y}$.

    \item The functor $G$ is a left Kan extension of $F$ along $f$.

    \item The natural transformation $\eta$ is a unit map.
  \end{itemize}
\end{definition}

\begin{example}
  \label{eg:strictly_commuting_left_kan}
  Any commuting simplex $\Delta^{2}_{\sharp} \to \ICCat$ such that $\Delta^{\{0,1\}}$ is mapped to an equivalence is left Kan.
\end{example}
...
\begin{definition}
  Let $X$, $Y$, and $\category{C}$ be $\infty$-categories, $f\colon X \to Y$, $F\colon X \to \category{C}$ and $G\colon Y \to \category{C}$ functors, and $\eta\colon F \to G \circ f$ a natural transformation.
  \begin{equation*}
    \begin{tikzcd}
      X
      \arrow[dr, swap, "f"]
    ...
    \end{tikzcd}
  \end{equation*}
  We say that $\eta$ \defn{exhibits $G$ as the left Kan extension of $F$ along $f$} if for each object $x \in X$,
\end{definition}
...
We will actually take this to be our working definition of the components of the unit map.

\begin{definition}
  \label{def:unit_map}
  Let $f \dashv g$ be adjoint functors. A morphism in $\category{\category{C}}$ is a \defn{component of a unit map} if it is adjunct to an equivalence in $\category{D}$ relative to $s = \id\colon \Delta^{1} \to \Delta^{1}$.
\end{definition}

\begin{note}
  In the context of Kan extensions along a functor $f$, with unit natural transformation $\eta\colon \id \Rightarrow f^{*}f_{!}$, one often calls the component of the unit map at a functor $F$, namely $\eta_{F}\colon F \to f^{*}f_{!}F$, simply the \emph{unit map,} dropping any mention of the word `component.' We will adopt this terminology.
\end{note}
...
%\begin{lemma}
%  Let $f\colon \category{C} \to \category{D}$ be a equivalence of quasicategories with homotopy inverse $f^{-1}$ (and natural isomorphism $c\colon f \circ f^{-1} \to \id_{\category{D}}$). Let $i\colon K' \hookrightarrow K$ be an inclusion of simplicial sets, $\alpha\colon K \to \category{C}$ and $\tilde{\alpha}'\colon K' \to \category{D}$ be maps, and denote $\alpha' = \alpha \circ i$. Suppose that $f^{-1} \circ \tilde{\alpha}' = \alpha'$. Then there exists a dashed extension
%  \begin{equation*}
%    \begin{tikzcd}
%      K'
%      \arrow[r, "\alpha'"]
...
%    \end{tikzcd}
%  \end{equation*}
%  such that $f \circ \alpha = \tilde{\alpha}$.
%\end{lemma}
%\begin{proof}
%  Using the maps
%  \begin{equation*}
%    K \overset{\alpha}{\to} \category{C} \overset{f}{\to} \category{D}
%  \end{equation*}
%  and
%  \begin{equation*}
%    K' \times \Delta^{1} \to \category{D} \times \Delta^{1} \to \category{D},
%  \end{equation*}
%  we can construct the solid filling problem
%  \begin{equation*}
%    \begin{tikzcd}
%      K \times \Delta^{\{0\}} \coprod_{K' \times \Delta^{\{0\}}} K' \times \Delta^{1}
%      \arrow[r]
%      \arrow[d, hook]
%      & \category{D}
%      \\
...
%    \end{tikzcd},
%  \end{equation*}
%  which has a dashed solution such that for each $k \in K$, $\{k\} \times \Delta^{1}$ is mapped to an equivalence in $\category{D}$. The restriction to $K \times \Delta^{\{1\}}$ gives us the extension we want.
%\end{proof}
...
We end this section with a few miscellaneous tricks which we can play with maps $\Delta^{n} \to \ICCat$ involving Kan extensions.
...
\end{lemma}
\begin{proof}
  Denote the image of the object $\Delta^{\{i\}}$ under $\sigma$ by $X_{i}$, the image of $\Delta^{\{i,j\}}$ by $f_{ji}$, and the image of $\Delta^{\{i,j,k\}}$ by $\alpha_{jki}$.

  If either $\sigma|\Delta^{\{0,1,2\}}$ or $\sigma|\Delta^{\{0,1,3\}}$ is left Kan, then we have adjoint functors
  \begin{equation*}
    (f_{10})_{!} : \Fun(X_{0}, X_{3}) \leftrightarrow \Fun(X_{1}, X_{3}) : (f_{10})^{*}.
  \end{equation*}

  We can extract from the data given to us by $\sigma$ a commuting square in $\Fun(X_{0}, X_{3})$
  \begin{equation*}
    \begin{tikzcd}[column sep=large, row sep=large]
      f_{30}
      \arrow[r, "\alpha_{310}"]
      \arrow[d, "\alpha_{320}"{swap}, "\simeq"]
      & f_{31}f_{10}
      \arrow[d, "\alpha_{321}f_{10}"]
      \\
      f_{32}f_{20}
      \arrow[r, "f_{32}\alpha_{210}"]
      & f_{32}f_{21}f_{10}
    \end{tikzcd}
  \end{equation*}
  which is adjunct to a commuting square in $\Fun(X_{1}, X_{3})$
  \begin{equation*}
    \begin{tikzcd}[column sep=large, row sep=large]
      (f_{10})_{!}f_{30}
      \arrow[r, "\eta"]
      \arrow[d, "(f_{10})!\alpha_{320}"{swap}, "\simeq"]
      & f_{31}
      \arrow[d, "\alpha_{321}", "\simeq"{swap}]
      \\
      (f_{10})_{!}f_{32}f_{20}
      \arrow[r, "\eta'"]
      & f_{32}f_{21}
  ...
  Note that $\eta$ is an equivalence if and only if $\eta'$ is an equivalence; thus, that $\alpha_{310}$ is a unit map if and only if $f_{32} \alpha_{210}$ is a unit map. But $f_{32}$ is an equivalence, so this in turn is true if and only if $\alpha_{210}$ is a unit map.
\end{proof}
