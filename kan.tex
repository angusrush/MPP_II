...
\begin{definition}
  Let $f\colon K \to \category{C}$ be a map between simplicial sets, where $\category{C}$ is an $\infty$-category. Let $c \in \category{C}$ be an object, and denote by $\underline{c}$ the constant functor $K \to \category{C}$ with value $c$. A natural transformation $f \Rightarrow \underline{c}$ \defn{exhibits $c$ as the colimit of $f$} if it is an initial object in the quasicategory $(f \downarrow \category{C})$ defined by the pullback diagram
  \begin{equation*}
    \begin{tikzcd}
