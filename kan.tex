...
\begin{definition}
  ...
  Let $\category{C}$ and $\category{C}$ be $\infty$-categories, and let $F\colon \category{C} \times \category{C} \to \category{C}$ be a functor. We say that \defn{$F$ preserves colimits in each slot} if and only if the following condition is satisfed:
  \begin{itemize}
