...
\begin{definition}
  \label{def:left_kan}
  We will call a $2$-simplex $\tau\colon \Delta^{2}_{\flat} \to \ICCat$ \defn{left Kan} if it is of the form
  \begin{equation*}
    \begin{tikzcd}
      X
      \arrow[dr, swap, "f"]
...
\begin{theorem}
  ...
  Let $\tau\colon \Delta^{2} \to \ICCat$ be left Kan. Then for each $n \geq 2$, every solid lifting problem of the form
  \begin{equation}
...
\begin{equation*}
  ...
  \end{tikzcd}
\end{equation*}
in $\Fun(X, \category{C})$.\footnote{In particular, if we take $g = \id$ and $\beta$ to be a natural isomorphism, we recover the classical universal property satisfied by left Kan extension.}
