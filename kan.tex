...
The following is an easy consequence of \cite[Remark~7.3.1.11]{kerodon}.
\begin{proposition}
...
\begin{equation*}
  ...
  \end{tikzcd}
\end{equation*}
where the diagram in question is now a diagram of $\SSetmk$-enriched categories (where the mapping spaces of $\C[\Lambda^{n+1}_{0}]$ and $\C[\Delta^{n}]$ are taken to carry the flat marking). In order to solve such lifting problems, we need to fill the missing data in each mapping space. As we will see in \hyperref[ssc:left_kan_implies_globally_left_kan]{Subsection~\ref*{ssc:left_kan_implies_globally_left_kan}}, these fillings take the form of filling a cube $(\Delta^{1})^{n}$ relative to its boundary missing one face.
