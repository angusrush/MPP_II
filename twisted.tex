...
\begin{equation*}
  ...
  \end{tikzcd}.
\end{equation*}
In order to show that $p$ is a cocartesian fibration, it will help us to understand the map $p''$. To do this, we will have to define several cosimplicial objects.

\subsubsection{Cosimplicial objects}
\label{sss:cosimplicial_objects}

The $n$-simplices of $\category{R}$ are given by maps
\begin{equation*}
  Q(\Delta^{n}) = (\Delta^{n} \star (\Delta^{n})\op)_{\dagger} \to \ICCat
\end{equation*}
such that
\begin{itemize}
  \item each object in $\Delta^{n}$ is sent to a space, and

  \item each morphism in $(\Delta^{n})\op$ is mapped to an equivalence in $\CC$.
\end{itemize}
...
\begin{definition}
  We denote by $\heartsuit$ the marking on $\Delta^{n} \star (\Delta^{n})\op$ consisting of all morphisms belonging to $(\Delta^{n})\op$, and by $(\Delta^{n} \star (\Delta^{n})\op)^{\heartsuit}_{\dagger}$ the corresponding marked-scaled simplicial set. We define a cosimplicial object
  \begin{equation*}
...
We have defined, for each $n \geq 0$, inclusions of marked-scaled simplicial sets
\begin{equation*}
  v_{n}\colon \tilde{J}(n) \to \tilde{R}(n).
\end{equation*}
We will show in \hyperref[ssc:the_fibration]{Subsection~\ref*{ssc:the_fibration}} that each $a_{n}$ is an equivalence. However, extreme care is warranted: \emph{the $v_{n}$ are not the components of a natural transformation $u\colon \tilde{J} \Rightarrow \tilde{R}$!} The necessary squares simply do not commute for morphisms $\phi\colon [m] \to [n]$ in $\Delta$ such that $\phi(0) \neq 0$.

We will simply define this problem away. Denote by $\mathring{\Delta}$ the subcategory of $\Delta$ on morphisms $[m] \to [n]$ which send $0 \mapsto 0$. Denote by $I$ the inclusion $\mathring{\Delta} \hookrightarrow \Delta$. The morphisms $v_{n}$ \emph{do} form a natural transformation $v\colon \tilde{J} \circ I \Rightarrow \tilde{R} \circ I$:
\begin{equation*}
...
Our goal in the remainder of this section is to show that each component $v_{n}$ is an equivalence in $\SSetms$.
...
\begin{proof}
  ...
  \end{equation*}
  Here, the map $v_{n}''$ is a pushout along the inclusion $(\Delta^{\{n\}})^{\sharp}_{\sharp} \hookrightarrow (\Delta^{n})^{\sharp}_{\sharp}$, which is marked-scaled anodyne. It remains to show that each of the maps $v_{n}'$ is marked-scaled anodyne.
  ...
  We now suppose that the result holds true for $n-1$; that is, that the maps $i^{n-1}_{k}$ are marked-scaled anodyne for all $0 \leq k \leq n-1$. We aim to show that each $i^{n}_{k}$ is marked-scaled anodyne for each $0 \leq i \leq n$. 
  ...
  Now we may assume that $n \geq 2$. In this case, we can write $i^{n}_{n}\colon M^{n}_{n} \hookrightarrow \Delta^{2n+1}$ as a simplicial inclusion
  \begin{equation*}
...
\begin{equation*}
  ...
  \end{tikzcd},
\end{equation*}
where each square is pullback, is a cocartesian fibration, and that the the $p$-cocartesian edges correspond to left Kan extensions. Our plan of attack is as follows.
\begin{enumerate}
  \item We will first specify, in \hyperref[eg:left_kan_morphism]{Example~\ref*{eg:left_kan_morphism}}, a class of morphisms in $\LS(\category{C})$, so-called \emph{left Kan morphisms,} which we claim are precisely the $p$-cocartesian morphisms.

  \item We will show that left Kan morphisms are in sufficient supply; that is, that given any morphism $f\colon X \to Y$ in $\S$ and a $\category{C}$-local system $\mathcal{F}\colon X \to \category{C}$, there exists a left Kan morphism $\hat{f}\colon \mathcal{F} \to f_{!}\mathcal{F}$ in $\LS(\category{C})$ whose image under $p$ is $f$.

  \item We will show that left Kan morphisms really are $p$-cocartesian.

  \item This, together with REF will imply that any $p$-cocartesian morphism is left Kan.
\end{enumerate}

\begin{definition}
  \label{def:admissible}
  A $(2n+1)$-simplex $\sigma\colon \Delta^{2n+1}_{\dagger} \to \ICCat$ will be called \defn{admissible} if the following conditions are satisfied.
  \begin{itemize}
...
The fibration $p\colon \LS(\category{C}) \to \S$ has enough left Kan lifts. More concretely, we have the following.
...
\begin{corollary}
  ...
  The map $p\colon \LS(\category{C}) \to \S$ admits enough left Kan lifts. More precisely, any lifting problem
  \begin{equation*}
    \begin{tikzcd}
      (\Delta^{\{0\}})^{\flat}
      \arrow[r]
...
\begin{proof}[Proof of Corollary \ref{cor:cocartesian_fibration}]
  We know that $p$ is a cartesian fibration, hence certainly an inner fibration. We need to show that $p$ has enough cocartesian lifts, i.e.\ that Any such lifting problem is adjunct to a lifting problem of the form
  \begin{equation}
  ...
  \end{equation*}
    ...
    The map $u_{1}$ is a pushout along the map $(\Delta^{\{1\}}, \flat, \flat) \hookrightarrow (\Delta^{1}, \sharp, \flat)$, which is marked-scaled anodyne of type \ref{item:outerms}. It is thus a weak equivalence. The map $v_{1}$ is a weak equivalence by \hyperref[lemma:lower_morphism_equivalence]{Lemma~\ref*{lemma:lower_morphism_equivalence}}. Thus, by \cite[Prop.~A.2.3.1]{highertopostheory}, in order to find a lift as in \hyperref[eq:unmarked_lifting_problem]{Equation~\ref*{eq:unmarked_lifting_problem}} it will suffice to find a lift
  \begin{equation*}
    \begin{tikzcd}
      (\Delta^{\{0,1\}} \cup \Delta^{\{0,3\}}, \flat, \flat)
      \arrow[r]
