...
\begin{definition}
  Let $p\colon \CC \to \DD$ be a functor between $\infty$-bicategories. We will call an edge $e\colon \Delta^{1} \to \CC$ \defn{outer locally cocartesian} if it is a cocartesian edge in the map $\category{M} \to \Delta^{1}$ defined by the pullback
  \begin{equation*}
    \begin{tikzcd}
      \category{M}
      \arrow[r]
...
Note that if an edge in $\CC$ is outer $p$-cocartesian, then it is
